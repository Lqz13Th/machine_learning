...
def factor_oi_change(df):
    return df['open_interest_data_sumOpenInterest'].pct_change()

def factor_oi_value_change(df):
    return df['open_interest_data_sumOpenInterestValue'].pct_change()

def factor_short_term_volatility(df, window=12):
    # 使用过去12个5分钟的收益标准差来衡量短期波动性
    return df['open_interest_data_sumOpenInterest'].pct_change().rolling(window).std()
...
def factor_buy_vol_pct_change(df):
    return df['trade_taker_long_short_ratio_data_buyVol'].pct_change()

def factor_sell_vol_pct_change(df):
    return df['trade_taker_long_short_ratio_data_sellVol'].pct_change()
